...
class WindowClass(QMainWindow, from_class):
    def __init__(self):
        ...
        self.result_screen.textChanged.connect(self.display_digit)
    ...
    # self.result_screen 값이 변할 때마다 해당 함수 실행
    def display_digit(self):
        if self.digit_result == "-0":
            str_format = self.digit_result
        else:
            try:
                str_format = str(format(int(self.digit_result), ','))
            except:
                str_format = str(format(float(self.digit_result), ','))

            if str_format[-2:] == ".0":
                # str_format = str(formatted)
                str_format = str_format[:-1]

        self.result_screen.setText(str(str_format))

        # str_digit = str(digit_result)
        # if '.' in str_digit:
        #     idx = str_digit.index('.')
        #     str_int = str_digit[:idx]
        #     str_demical = str_digit[idx:]
        # else:
        #     str_int = str_digit

        # len_digit = len(str_digit) - 1
        # if len_digit // 3 > 0:
    ...
    def interim_Calculator(self):
        ...
        if self.calc_list[-1] in ['+', '-']:
            ...
            interim_str = interim_str.replace(',', '')
    ...
    def equal_btn_Clicked(self):
        if self.is_last_input_operator == False:  # 마지막 입력이 연산자가 아닌 경우 즉, 숫자로 끝난 경우
            ...
            self.calc_str = self.calc_str.replace(',', '')
